...
class ExperimentConfig:
    ...
    device: str = "cpu"
    top_rank: int = 4
    output_root: str = "proposal3_outputs_v6"
...
def compute_metrics(model: TwoLayerSoftplus, X: torch.Tensor, y: torch.Tensor, lambda_B: float,
    ...
    if rank_rel > 0 and math.isfinite(d_eff) and abs(d_eff) > 1e-16:
        ...
        alpha_rel = safe_op_norm_symmetric(U_rel.T @ ((X @ X.T) / d_eff) @ U_rel - I_rel)
...
def train_one(cfg: ExperimentConfig) -> Dict[str, object]:
    ...
    best_state_for_plots = None

    def maybe_update_best(metrics: Dict[str, float]) -> None:
        nonlocal best_stationarity, best_weighted, best_raw_conditioning, best_state_for_plots
        if metric_better(metrics, best_stationarity, key="delta_stationary_op"):
            best_stationarity = copy.deepcopy(metrics)
            best_state_for_plots = {k: v.detach().clone() for k, v in model.state_dict().items()}
    ...
    if best_state_for_plots is not None:
        best_model = TwoLayerSoftplus(cfg, dtype=dtype, device=device)
        best_model.load_state_dict(best_state_for_plots)
    else:
        best_model = model

    with torch.no_grad():
        Q_best = compute_q(best_model, X)
        M_best = (Q_best.T @ Q_best) / float(cfg.n)
        H_best = best_model.B.T @ best_model.B
        G_best = best_model.B.T @ M_best @ best_model.B

    save_history(out_dir, cfg, history, dataset_diag, best_stationarity, best_weighted, best_raw_conditioning)
    plot_training_curves(out_dir, cfg, history)
    plot_eigen_comparison(out_dir, H_best, G_best)
    save_final_matrices(out_dir, H_best, G_best)
...
def save_history(out_dir: Path, cfg: ExperimentConfig, history: List[Dict[str, float]], dataset_diag: Dict[str, float],
                 best_stationarity: Dict[str, float] | None, best_weighted: Dict[str, float] | None,
                 best_raw_conditioning: Dict[str, float] | None) -> None:
    payload = {
        "config": asdict(cfg),
...
def plot_training_curves(out_dir: Path, cfg: ExperimentConfig, history: List[Dict[str, float]]) -> None:
    ...
    ax = axes[0, 1]
    ax.plot(steps, [max(abs(h["beta_fit"]), 1e-16) if math.isfinite(h["beta_fit"]) else float("nan") for h in history], label="|beta_fit|")
    ax.plot(steps, [max(h["resid_rms"], 1e-16) if math.isfinite(h["resid_rms"]) else float("nan") for h in history], label="resid_rms")
    ax.set_title("Raw-conditioning proxy")
    ax.set_yscale("log")
    ax.legend()
    ...
    ax.set_title("Hidden-space closure")
    ax.set_yscale("log")
    ax.legend()

    ax = axes[1, 1]
    ax.plot(steps, [max(h["gamma_tilde_eff_op"], 1e-16) if math.isfinite(h["gamma_tilde_eff_op"]) else float("nan") for h in history], label="gamma_tilde_eff_op")
    ...
    ax.set_title("Weighted vs raw square law")
    ax.set_yscale("log")
    ax.legend()
...
def plot_eigen_comparison(out_dir: Path, H: torch.Tensor, G: torch.Tensor) -> None:
    ...
    plt.title("Best-stationarity spectral comparison")
    plt.legend()
    plt.tight_layout()
...
def default_configs() -> List[ExperimentConfig]:
    ...
    for noise in noise_levels:
        suffix = str(noise).replace(".", "p").replace("-", "m")
        runs.append(ExperimentConfig(
            name=f"isotropic_noise_{suffix}",
            seed=0,
            anisotropic=False,
            noise_std=noise,
            beta_threshold=1e-5,
        ...
        runs.append(ExperimentConfig(
            name=f"anisotropic_noise_{suffix}",
            seed=0,
            anisotropic=True,
            spectrum_min=1e-2,
            noise_std=noise,
...
def main() -> None:
    ...
    for row in all_results:
        stat = row["best_metrics_by_stationarity"]
        raw = row["best_metrics_by_raw_conditioning"]
        if raw is None:
            raw_msg = "no checkpoint met beta threshold"
        else:
            raw_msg = f"best raw gamma={raw['gamma_eff_op']:.3e}, beta={raw['beta_fit']:.3e}, step={raw['step']}, phase={raw['phase']}"
        print(
            f"- {row['name']}: "
            f"best stat delta_eff={stat['delta_eff_op']:.3e}, gamma_tilde={stat['gamma_tilde_eff_op']:.3e}, "
            f"gamma_raw={stat['gamma_eff_op']:.3e}, beta={stat['beta_fit']:.3e} | {raw_msg}"
        )
