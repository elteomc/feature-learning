...
\begin{frame}{Where things stand}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      \textbf{Open}
      \begin{itemize}\setlength\itemsep{1pt}
        \item \emph{Isolate a pure pair failure}: every pair-geometry attack so far self-aligns. We still lack a family that degrades the weighted law \emph{without} bending beta (e.g.\ a large shifted-input cluster).
      ...
      \begin{ibox}{Take-home}
        Raw NFA is useful but \emph{regime-dependent}: at convergence the primitive object is the \emph{residual-weighted} law, and raw AGOP is its conditioned consequence.
      \end{ibox}
      \vfill\null
    \end{column}
  \end{columns}
